...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        if (!["TAG"].some(ss => member.user.username.toLowerCase().includes(ss)) && member.user.discriminator !== "0000" && !message.guild.members.cache.get(member.id).roles.cache.has('TAGROL' && 'BOOSTROL')) {
            message.react("❌")
        ...
        db.push(`isimler_${member.id}`, ` \`${name} | ${age}\` (<@ERKEKROL1>), (<@ERKEKROL>)`);
        ...
        message.react("✅")
        message.channel.send(embed2.setDescription(`${member} adlı kullanıcı\`${name} | ${age}\` isminde  kayıt edildi! (<@826727386543423534>), (<@826727386543423533>)`)
